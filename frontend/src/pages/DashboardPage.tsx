  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <button
        ...
        className="text-gray-500 hover:text-white text-xs mb-6 block"
      >
        ← Overview
      </button>

      <h1 className="text-2xl font-bold text-white mb-6">
        {summary?.name ?? 'Repository'} — Dashboard
      </h1>
      ...
      <div className="grid grid-cols-4 gap-3 mb-8">
        {[
          {
            label: 'Files',
            value: summary?.stats?.file_count ?? 0,
          },
          {
            label: 'LOC',
            value:
              (summary?.stats?.total_loc ?? 0) >= 1000
                ? `${(
                    (summary?.stats?.total_loc ?? 0) / 1000
                  ).toFixed(1)}k`
                : summary?.stats?.total_loc ?? 0,
          },
          {
            label: 'Languages',
            value: Object.keys(
              ...
              <table className="w-full text-sm">
                ...
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-left text-xs text-gray-500 px-4 py-2.5 font-medium">
                      File
                    </th>

                    <th className="text-right text-xs text-gray-500 px-4 py-2.5 font-medium">
                      LOC
                    </th>
                  ...
                  {(summary?.top_files ?? []).map((f, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 text-right text-gray-400 text-xs">
                        {f?.loc?.toLocaleString?.() ?? 0}
                      </td>

                      <td className="px-4 py-2.5 text-right text-xs">
                        <span className="text-gray-500">
                          {LANG_ICONS[f?.language] ?? '🔵'}{' '}
                          {f?.language ?? 'Unknown'}
                        </span>
                      </td>
                    </tr>
                ...
                {(summary?.file_summaries ?? [])
                  ...
                  .map((fs, i) => (
                    <div key={i} className="card">
                      ...
                      <div className="text-xs text-brand-400 font-mono mb-1">
                        {fs?.path ?? 'Unknown'}
                      </div>

                      <div className="text-gray-300 text-sm">
                        {fs?.summary ?? 'No summary'}
                      </div>
            ...
            <div className="card">
              ...
              <p className="text-gray-300 text-sm leading-relaxed mb-4">
                {summary?.onboarding?.summary ??
                  'No onboarding summary available'}
              </p>

              <div className="space-y-2">
                {(summary?.onboarding?.steps ?? []).map(
                  (step, i) => (
                    <div
                      key={i}
              ...
              {(summary?.onboarding?.entry_points ?? []).length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-800">
                  ...
                  {(summary?.onboarding?.entry_points ?? []).map(
                    (ep, i) => (
                      <div
                        key={i}
                        className="text-xs text-yellow-400 font-mono mb-1"
                      >
                        ★ {ep}
                      </div>
                    )
